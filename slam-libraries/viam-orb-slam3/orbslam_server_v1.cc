   ...
   public:
    ...
    ::grpc::Status GetMap(ServerContext *context, const GetMapRequest *request,
        ...
        if (mime_type == "image/jpeg") {
            ...
            if (size > 1) {
                ...
                BOOST_LOG_TRIVIAL(debug) << "[1] Dimensions are:" << minX << " -> " << maxX << ", " << minZ << " -> " << maxZ;
                float minCalX =  xMedian - sigmaLevel*xSD;  
                float maxCalX =  xMedian + sigmaLevel*xSD; 

                float minCalZ = zMedian - sigmaLevel*zSD;  
                float maxCalZ = zMedian + sigmaLevel*zSD;   
                ...
                BOOST_LOG_TRIVIAL(debug) << "[2] Dimensions are:" << minX << " -> " << maxX << ", " << minZ << " -> " << maxZ; 
            }
    ...
    void process_rgbd_offline(ORB_SLAM3::System *SLAM) {
        ...
        std::vector<std::string> filesRGB = listFilesInDirectoryForCamera(
            path_to_data + strRGB, ".png", camera_name);
        if (filesRGB.size() == 0) {
            BOOST_LOG_TRIVIAL(debug) << "Location: " << path_to_data;
