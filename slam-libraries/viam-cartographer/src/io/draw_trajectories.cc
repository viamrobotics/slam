...
cartographer::io::UniqueCairoSurfacePtr DrawTrajectoryNodes(
    ...
    if (trajectory_nodes.size() == 0) {
        return cartographer::io::MakeUniqueCairoSurfacePtr(
            cairo_get_target(cr));
    }
    ...
    for (const auto& node : trajectory_nodes.trajectory(last_traj)) {
        if (print_first) {
            ...
            const Eigen::Vector3d pixel = t_global_pose.translation();
            double px_start =
                (slice_pose.translation().y() - pixel.y()) / resolution;
            double py_start =
                (slice_pose.translation().x() - pixel.x()) / resolution;
            cairo_arc(cr, px_start, py_start, kTrajectoryEndMarkers, 0,
                      2 * M_PI);
            cairo_fill(cr);
            print_first = false;
        }
